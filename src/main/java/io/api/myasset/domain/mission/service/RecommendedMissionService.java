...
public class RecommendedMissionService {
    ...
    public List<RecommendedMissionResponse> getRecommendedMissions(Long userId) {
        ...
        if (!Boolean.TRUE.equals(acquired)) {
            log.info("[RecommendedMission] 동시 GPT 요청 감지 - userId={}, 캐시 재확인", userId);

            List<RecommendedMissionResponse> retryCached = readCache(userId, today);
            if (retryCached != null) {
                return retryCached;
            }

            throw new BusinessException(MissionError.GENERATING_RECOMMENDED_MISSION);
        }
